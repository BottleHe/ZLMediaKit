...
void MediaSink::checkTrackIfReady() {
    TraceL << "checkTrackIfReady called, all_track_ready=" << _all_track_ready << ", callback_size=" << _track_ready_callback.size() << ", track_map.size=" << _track_map.size() << ", max_track_size=" << _max_track_size;
    if (!_all_track_ready && !_track_ready_callback.empty()) {
        for (auto &pr : _track_map) {
            TraceL << "checkTrack callback loop: index=" << pr.first << ", have_frame=" << pr.second.second << ", ready=" << pr.second.first->ready() << ", codec=" << pr.second.first->getCodecName();
...
void MediaSink::addTrackCompleted() {
    TraceL << "MediaSink::addTrackCompleted called, track_map.size=" << _track_map.size() << ", all_track_ready=" << _all_track_ready;
    if (!_track_map.empty()) {
...
void MediaSink::emitAllTrackReady() {
    TraceL << "MediaSink::emitAllTrackReady called, all_track_ready=" << _all_track_ready << ", track_map.size=" << _track_map.size();
    if (_all_track_ready) {
        return;
    }
...
void MediaSink::onAllTrackReady_l() {
    TraceL << "MediaSink::onAllTrackReady_l called";
